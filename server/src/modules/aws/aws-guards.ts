...
import {
  hasUnacknowledgedExpiredReagents,
  isInstrumentExpired,
  parseReagentsUsed,
  startOfUtcDay,
...
export function assertNotSameAsAnalyst(section: AwsSectionDetail, actor: JwtAccessPayload): void {
  if (section.analyzedById && section.analyzedById === actor.userId) {
...
export function assertSectionHasConclusion(section: AwsSectionDetail): void {
  ...
  const resultType =
    section.specDocumentTest?.resultType ?? section.testParameter.resultType;
  if (resultType === ResultType.QUALITATIVE) {
    const obs = (section.observations ?? {}) as { passFail?: string };
    if (obs.passFail) return;
  }
...
export async function assertExpiryAcknowledged(section: AwsSectionDetail): Promise<void> {
  ...
  if (section.instrumentId && section.instrument) {
    if (isInstrumentExpired(section.instrument, today) && !section.instrumentExpiredAck) {
  ...
  if (hasUnacknowledgedExpiredReagents(entries, reagentMap, today)) {
    throw AppError.expiredNotAcknowledged();
  }
}

export function assertOosAcknowledged(section: AwsSectionDetail): void {
  if (section.oosDetected && !section.oosAcknowledged) {
